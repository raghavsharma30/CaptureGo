...
void Game::run() {
    ...
    while (true) {
        ...
        if (winner != Piece::NONE) {
            ...
            Network::sendData(sockets[1], msg);
            // Ask for rematch
            Network::sendData(sockets[0], "REMATCH?");
            Network::sendData(sockets[1], "REMATCH?");
            string response0 = Network::receiveData(sockets[0]);
            string response1 = Network::receiveData(sockets[1]);
            if (response0 == "YES" && response1 == "YES") {
                // Start a new game directly
                Game* rematch = new Game(players[0], players[1]);
                thread([rematch] {
                    rematch->run();
                    delete rematch;
                }).detach();
                return; // Skip notifying condition variables
            }
            break;
        }
